...
{
	int reactionDelay;     // Задержка реакции (кадры)
	int framesCounter;     // Счетчик кадров
	int targetY;           // Целевая позиция
...
paddle_bot::paddle_bot()
{
	...
	framesCounter = 0;
	targetY = HEIGHT / 2;
	reactionDelay = 15 + rand() % 10;  // 15-25 кадров (0.25-0.42 сек)
	speed_y = 7;                       // Медленнее игрока
}
...
void paddle_bot::Movement(ball* p_bl)
{
	...
	// Обновляем цель только после задержки
	if (framesCounter >= reactionDelay)
	{
		framesCounter = 0;
		int errorRange=80;
		int error = (rand() % (errorRange * 2)) - errorRange;		// Случайная ошибка
		int predictedY = p_bl->y + p_bl->speed_y * 15;		// Предсказываем позицию мяча с учетом его скорости
		targetY = predictedY + error;		// Устанавливаем цель с ошибкой
	}
	...
	if (distance > 10)
	{
		// Движемся с разной скоростью в зависимости от расстояния
		int speed = speed_y;

		// Если близко к цели - двигаемся медленнее (плавность)
		if (distance < 30)
			speed = speed_y / 2;
